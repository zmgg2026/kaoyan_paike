...
class ReleaseStaticTest(unittest.TestCase):
    ...
    def test_product_catalog_logic_lives_outside_admin_server(self) -> None:
        ...
        self.assertIn("STAGE_ORDER_PROFILES", product_catalog_source)
        self.assertIn("STAGE_RANK_PROFILES", product_catalog_source)
        self.assertIn("from scripts.product_catalog import infer_stage_order_from_context", scheduler_source)
        self.assertIn("from scripts.product_catalog import DEFAULT_STAGE_ORDER", schedule_outputs_source)
        self.assertIn("from scripts.window_utils import SEASON_WINDOW_ORDER", schedule_outputs_source)
        self.assertIn("from scripts.product_catalog import stage_rank_map_from_context", quality_repair_source)
        self.assertIn("from scripts.product_catalog import stage_rank_map_from_context", extreme_weeks_repair_source)
        ...
        self.assertIsNone(re.search(r"(?m)^STAGE_ORDER_PROFILES\s*=\s*\[", scheduler_source))
        self.assertIsNone(re.search(r"(?m)^STAGE_ORDERS\s*=\s*\{", quality_repair_source))
        self.assertNotIn('stage_order = {"基础": 0, "强化": 1, "冲刺": 2}', extreme_weeks_repair_source)
        self.assertNotIn('["寒假", "春季", "暑假", "秋季", "一轮", "二轮", "三轮", "四轮", "基础", "强化", "冲刺"]', schedule_outputs_source)
