...
class SchedulingPipelineTest(unittest.TestCase):
    ...
    def test_build_lookups_prefers_master_rows_and_backfills_missing_options(self) -> None:
        ...
        self.assertEqual(lookups["product_projects"], data_admin_server.PRODUCT_PROJECT_OPTIONS)
        ...
        self.assertEqual(lookups["teacher_subject_types"], data_admin_server.TEACHER_SUBJECT_TYPE_OPTIONS)
        self.assertEqual(lookups["teacher_contract_statuses"], data_admin_server.TEACHER_CONTRACT_STATUS_OPTIONS)
        self.assertEqual(lookups["teacher_employment_statuses"], data_admin_server.TEACHER_EMPLOYMENT_STATUS_OPTIONS)
        self.assertEqual(lookups["subjects"], ["政治", "数学"])
        self.assertEqual(lookups["window_names"], ["春季", "暑假"])
        self.assertNotIn("quarters", lookups)
        self.assertEqual(lookups["season_window_order"], ["寒假", "春季", "暑假", "秋季"])
        self.assertEqual(lookups["stage_order"], data_admin_server.DEFAULT_STAGE_ORDER)
        self.assertEqual(lookups["stages"], ["基础", "强化"])
        self.assertEqual(set(lookups["public_teacher_subjects"]), {"英语", "政治", "数学", "语文"})
