...
- `python3 -m unittest discover -v` 通过；具体测试数量以本次命令输出为准。
...
- `bash scripts/verify_release.sh` 可一键执行脚本语法检查、发布包内 Python 脚本编译、根目录和 `scripts/` CLI 入口 `--help` 冒烟检查、发布包内容审计、临时 release zip 审计、单元测试、核心 JSON 样例、公开 CSV 最小闭环、覆盖审计和质量审计。
...
- 若要用本部门真实数据演示完整排课，请在本地另行执行预检并补齐报告里的业务数据缺口；真实 `data/` 不进入 GitHub 发布包。
...
git archive --format=zip -o /tmp/ai-scheduling-project-release-<提交号>.zip HEAD
python3 scripts/audit_release_package.py --zip /tmp/ai-scheduling-project-release-<提交号>.zip
```
