...
第一次使用时，不要先研究全部页面。先把下面三件事分清：

- 数据模板：19 张业务表，是正式导入和排课的标准数据。sheet 名和程序字段名不能改。
- 排课运行维护：日常入口，用来下载或生成模板、上传前校验、生成课表、查看结果。
- 发布复用中心：验收后的交付入口，用来看只读课表、排课报告和复用资料；不在这里录入数据或重新排课。
...
项目已经发布到 GitHub。其他部门可以把仓库链接发给 Workbuddy、Qclaw、Marvis、Codex、Claude Code 等 AI 工具，让 AI 先下载项目、安装依赖并启动本地后台：
...
如果你让 AI 助手代跑，建议直接给它三件材料：GitHub 链接、本部门 ERP 导出的 Excel/CSV、这份攻略。让它先跑 `verify_release.sh` 和公开最小示例，再处理你的真实数据；不要一开始就让它改规则或正式排课。
...
| 这份攻略 | 约束 AI 的处理顺序和验收口径 | 要求先预检，缺口只生成补录清单，不要猜老师或跳过硬规则 |
...
```text
请下载并运行这个 AI 排课项目。先按 README 跑通 verify_release.sh 和 examples/csv_minimal，再根据我提供的 ERP 导出整理 19 张模板表。正式排课前必须先做上传前校验；如出现缺老师、缺产品映射、无可用课节等问题，只生成补录清单和说明，不要擅自猜测老师或跳过硬约束。
```
